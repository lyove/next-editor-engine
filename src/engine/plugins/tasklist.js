...
export default {
  initialize: function () {
    this.schema.add([
      ...
      {
        li: {
          class: ["xEditor-list-node", "xEditor-list-task"],
        },
      },
    ]);
    // 创建命令
    ...
    this.on("paste:before", (fragment) => {
      ...
      nodes.forEach((node) => {
        node = getNodeModel(node);
        if (node.name === "li" && node.hasClass("xEditor-list-task")) {
          node.closest("ul").addClass("xEditor-list");
    ...
    this.on("paste:insert", () => {
      ...
      if (nextBlock && nextBlock.find("li.xEditor-list-task").length > 0) {
        nextBlock.find("li.xEditor-list-task").each((node) => {
          node = getNodeModel(node);
          if (0 === node.find("[data-xEditor-card=checkbox],[data-ready-card=checkbox]").length) {
            prependReadyCheckbox.call(this, node);
          }
        });
      }

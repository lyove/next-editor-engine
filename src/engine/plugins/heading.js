...
import { CURSOR, ANCHOR, XEDITOR_ELEMENT } from "../constants/bookmark";
...
const updateAnchorIconPosition = function () {
  const button = getNodeModel(".xEditor-anchor-button");
...
const showAnchorIcon = function () {
  const range = this.change.getRange();
  let button = getNodeModel(".xEditor-anchor-button");
  const block = getNodeModel(range.startContainer).closest("h1,h2,h3,h4,h5,h6");

  if (
    block.length === 0 ||
    (button.length > 0 && button.find(".xEditor-icon-".concat(block.name)).length === 0)
  ) {
    button.remove();
  }
  ...
  if (button.find(".xEditor-icon-".concat(block.name)).length > 0) {
    updateAnchorIconPosition.call(this);
    return;
  }

  button = getNodeModel(
    '<span class="xEditor-anchor-button"><span class="xEditor-icon xEditor-icon-'.concat(
      block.name,
      '"></span></span>'
    )
  );
  ...
  button.addClass("xEditor-anchor-button-active");
...
const beforeProcess = function () {
  ...
  blocks.forEach((block) => {
    ...
    if ([CURSOR, ANCHOR].includes(first.attr(XEDITOR_ELEMENT))) {
      first = first.next();
    }
...
const afterProcess = function (start) {
  ...
  blocks.forEach((block) => {
    fetchAllChildren(block).forEach((node) => {
      node = getNodeModel(node);
      if (
        (node.name === "strong" || node.name === "span") &&
        /^xEditor-fontsize-\d+$/.test(node[0].className)
      ) {
        unwrapNode(node[0]);
        return;
      }
      ...
      if (node.name === "span") {
        const match = /(?:^|\b)(xEditor-fontsize-\d+)(?:$|\b)/.exec(node[0].className);
        if (match) {
    ...
    if (parent && ["ul", "ol"].includes(parent.name)) {
      parent.find("[data-xEditor-card=checkbox]").remove();
      unwrapNode(parent[0]);
    }
  });
  moveToBookmark(range, bookmark);

  ...
  initialize: function () {
    ...
    this.schema.add({
      blockquote: {
        class: [
          "xEditor-alert",
          "xEditor-alert-info",
          "xEditor-alert-warning",
          "xEditor-alert-danger",
          "xEditor-alert-success",
          "xEditor-alert-tips",
        ],
      },
    });
    ...
    this.command.add(PLUGIN_NAME, {
      queryState: () => {
        ...
        return blocks[0].closest(".xEditor-alert").length > 0;
      },
      execute: (type) => {
        ...
        if (this.command.queryState(PLUGIN_NAME)) {
          ...
          const blocks = this.change.blocks;
          const alertBlock = blocks[0].closest(".xEditor-alert");
        ...
        // 添加警告提示
        this.change.wrapBlock(
          '<blockquote class="xEditor-alert xEditor-alert-'.concat(type, '" />')
        );
      },
    });
    // Markdown 快捷键

...
const removeAllLinkToolbar = function () {
  ...
  getNodeModel("div[data-xEditor-element=embed-link-toolbar]").remove();
  toolbarList = [];
};
...
const insertLink = function (url, text) {
  ...
  const linkNode = getNodeModel(
    '<a href="'
      .concat(sanitizeUrl(url), '"')
      .concat(target, ">")
      .concat(text || url, "</a>")
  );
...
const createLinkToolbar = function (linkNode) {
  ...
  if (lastNode) {
    const node = getNodeModel('<div data-xEditor-element="embed-link-toolbar" />');
    ...
    if (linkNode.closest(".xEditor-card-block-max").length > 0) {
      toolbar.root.addClass("xEditor-link-toolbar-in-max");
    }
    toolbar.render(node);
...
const bindEvents = function () {
  ...
  this.domEvent.onDocument("click", (e) => {
    if (getNodeModel(e.target).closest(".xEditor-button-link,.xEditor-embed-toolbar").length > 0) {
      return;
    }
    removeAllLinkToolbar.call(this);
  });
  ...
  initialize: function () {
    ...
    this.command.add(PLUGIN_NAME, {
      execute: () => {
        ...
        this.history.stop();
        this.change.unwrapInline("<a />");
        this.change.wrapInline('<a href="" target="_blank" class="xEditor-link-marker" />');
        shrinkRange(range);
        this.change.select(range);
        this.history.save();
        let linkNode = getNodeModel(range.commonAncestorContainer);
        if (linkNode.name !== "a") {
          linkNode = linkNode.find(".xEditor-link-marker");
        }
        linkNode.removeClass("xEditor-link-marker");
        createLinkToolbar.call(this, linkNode);
        return linkNode;
      },
    });
...
export { removeAllLinkToolbar, removeLinkToolbar, createLinkToolbar, updateLink };

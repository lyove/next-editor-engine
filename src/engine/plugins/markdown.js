...
const processMark = function (e, range) {
  ...
  for (let i = 0; i < markTypeKeys.length; i++) {
    ...
    if (match) {
      ...
      if (
        markName === "" ||
        !this.command.queryEnabled(markName) ||
        !options.items[markName].enabled
      ) {
        return;
      }
      e.preventDefault();
...
const processLink = function (e, range) {
  ...
  if (match) {
    ...
    const linkNode = getNodeModel(
      '<a href="'.concat(sanitizeUrl(url), '"').concat(target, ">").concat(text, "</a>")
    );
    this.history.stop();
    // 移除 markdown 语法
...
const handleKeydownSpace = function (e) {
  ...
  if (
    ...
    this.command.queryEnabled("tasklist") &&
    options.items[type].enabled
  ) {
    e.preventDefault();
    removeLeftText(block, range);
    this.history.save();

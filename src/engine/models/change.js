...
function focusRang(range) {
  ...
  if (cardRoot) {
    ...
    if (
      node_center &&
      (!node.isElement() || node[0].parentNode !== cardRoot[0] || node.attr(CARD_ELEMENT_KEY))
    ) {
      const comparePoint = () => {
...
const selectComponent = (component, selected) => {
  ...
  if (selected) {
    if (!component.state.selected) {
      component.cardRoot.addClass("xEditor-selected");
      if (component.select) {
        component.select();
      }
    ...
    if (component.state.selected) {
      component.cardRoot.removeClass("xEditor-selected");
...
const initNativeEvents = function () {
  ...
  this.domEvent.onDocument("mousedown", (e) => {
    ...
    // 阅读模式节点
    if (target.closest(".xEditor-engine-view").length > 0) {
      return;
    }
    // 工具栏、侧边栏、内嵌工具栏的点击
    let node = target;
    while (node) {
      const attrValue = node.attr(XEDITOR_ELEMENT);
      ...
      if (this.activeCard && !isSameCard) {
        ...
        if (component) {
          if (component.unactivate) {
            component.cardRoot.removeClass("xEditor-activated");
        ...
        if (component) {
          ...
          if (!isSameCard) {
            ...
            if (
              ...
              component.constructor.autoSelected !== false &&
              (triggerType !== "click" || (component.state && !component.state.readonly))
            ) {
              this.selectCard(cardRoot);
            }
            if (component.activate) {
              component.cardRoot.addClass("xEditor-activated");

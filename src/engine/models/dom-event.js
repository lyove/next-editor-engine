...
class DOMEvent {
  ...
  isCardInput(e) {
    ...
    while (node) {
      ...
      if (node.hasClass("xEditor-embed-toolbar")) {
        return true;
      }
      node = node.parent();
    }
  ...
  onDrop(callback) {
    ...
    const removeCursor = () => {
      getNodeModel("body > div.xEditor-drop-cursor").remove();
    };
    const setCursor = () => {
      removeCursor();
      const targetCursor = getNodeModel('<div class="xEditor-drop-cursor" />');
    ...
    this.editArea.on("dragstart", (e) => {
      ...
      if (cardRoot) {
        ...
        dragImage = cardRoot.find("img.xEditor-drag-image");

        if (dragImage.length > 0) {
          dragImage = dragImage.clone();
        } else {
          dragImage = getNodeModel('<div class="xEditor-drag-image" />');
          dragImage.css({
    ...
    this.editArea.on("dragover", (e) => {
      ...
      const targetCursor = getNodeModel("body > div.xEditor-drop-cursor");

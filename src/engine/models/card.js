...
const dndTemplate = () => {
  return '\n    <div class="xEditor-card-dnd" draggable="true" contenteditable="false">\n      <div class="xEditor-card-dnd-trigger">\n        <span class="xEditor-icon xEditor-icon-drag"></span>\n      </div>\n    </div>\n  ';
};

const readCardToolTemplate = () => {
  return '\n    <div class="xEditor-card-read-tool"></div>\n  ';
};

const maximizeHeaderTemplate = (cfg) => {
  const back = cfg.back;
  return '\n    <div class="header clearfix" data-transient="true">\n      <div class="header-crumb header-crumb-sm">\n        <a class="split">\n          <span class="xEditor-icon xEditor-icon-arrow-left"></span>\n        </a>\n        <a>'.concat(
    back,
    "</a>\n      </div>\n    </div>\n  "
  );
};
...
class Card {
  ...
  getId(id, component) {
    ...
    while (cache[id] && cache[id].component !== component) {
      id = randomId();
    }
    return id;
  }
  ...
  showToolbar(cardRoot) {
    this.find(cardRoot, ".xEditor-card-dnd").addClass("xEditor-card-dnd-active");
    this.showCardToolbar(cardRoot);
  }
  // 显示卡片工具栏
  showCardToolbar(cardRoot) {
    const toolbarNode = this.find(cardRoot, ".xEditor-card-toolbar");
    toolbarNode.addClass("xEditor-card-toolbar-active");
    toolbarNode.addClass("xEditor-embed-toolbar-active");
  }
  // 隐藏工具栏
  hideToolbar(cardRoot) {
    this.find(cardRoot, ".xEditor-card-dnd").removeClass("xEditor-card-dnd-active");
    this.hideCardToolbar(cardRoot);
  }
  // 隐藏卡片工具栏
  hideCardToolbar(cardRoot) {
    const toolbarNode = this.find(cardRoot, ".xEditor-card-toolbar");
    toolbarNode.removeClass("xEditor-card-toolbar-active");
    toolbarNode.removeClass("xEditor-embed-toolbar-active");
  }
  // 设置卡片工具栏
  setToolbar(cfg) {
    ...
    if (engine) {
      ...
      config = config.map((item) => {
        if (item.type === "separator") {
          return {
            type: "node",
            node: getNodeModel('<span class="xEditor-embed-toolbar-item-split"></span>'),
          };
        }
      ...
      embedToolbar.root.addClass("xEditor-card-toolbar");
      ...
      config.map((item) => {
        const toolNode = getNodeModel(
          '\n          <span class="xEditor-icon xEditor-icon-'
            .concat(item.type, " xEditor-card-")
            .concat(item.type, '-trigger"></span>\n        ')
        );
  ...
  defalutMaximize(cfg) {
    ...
    cardRoot.addClass("xEditor-card-block-max");
  ...
  defalutRestore(cfg) {
    ...
    cardRoot.removeClass("xEditor-card-block-max");
    ...
    if (engine) {
      ...
      cardRoot.find(".xEditor-card-read-tool").hide();
    }
  ...
  replaceNode(node, name, value) {
    ...
    const html = transformCustomTags(
      '<card type="'.concat(type, '" name="').concat(name, '"></card>')
    );
  ...
  createComponent(cfg) {
    ...
    if (componentClass) {
      ...
      // 生成卡片容器
      const container =
        component.type === "inline" ? getNodeModel("<span />") : getNodeModel("<div />");
      container.attr(CARD_ELEMENT_KEY, "center");
      // 设置卡片只读属性

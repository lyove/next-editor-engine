...
import { XEDITOR_ELEMENT } from "../constants/bookmark";
    ...
    if (node.isHeading()) {
      ...
      if (childrenCount === 0) {
        ...
        if (
          childrenCount === 1 &&
          child.name === "span" &&
          ["cursor", "anchor", "focus"].indexOf(child.attr(XEDITOR_ELEMENT)) >= 0
        ) {
          node.prepend("<br />");
        }
      }
...
const setAttributes = function () {
  ...
  if (!editArea.hasClass("xEditor-engine")) {
    editArea.addClass("xEditor-engine");
  }
...
const removeAttributes = function () {
  ...
  if (this.card.closest(editArea)) {
    editArea.removeClass("xEditor-engine");
  }
};
...
const clickBottomArea = function (e) {
  if (getNodeModel(e.target).isRoot()) {
    ...
    if (lastBlock) {
      ...
      if (lastBlock[0].offsetTop + lastBlock[0].clientHeight > e.offsetY) {
        return;
      }
    }
...
class Engine {
  constructor(expr, options) {
    this.toolbar = {
      ...
      restore: function () {},
      disable: function () {}, // 在 xEditor-editor 里覆盖
    };
    this.sidebar = {
      set: function () {},
      close: function () {},
      restore: function () {}, // 在 xEditor-editor 里覆盖
    };
  ...
  getHtml(parser) {
    if (!parser) {
      throw new Error("Need to pass in a parser");
    }

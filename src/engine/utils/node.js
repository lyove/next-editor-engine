...
import { ANCHOR, FOCUS, CURSOR, XEDITOR_ELEMENT } from "../constants/bookmark";
...
export const removeMark = (node, styleName) => {
  if ("font-size" !== styleName) {
    ...
    node.attr("class", classVal.replace(/xEditor-fontsize-[0-9]{1,5}/, ""));
  }
};
...
export const sameMarkNode = (mark1, mark2) => {
  ...
  if (
    mark1Class &&
    mark2Class &&
    mark1Class.indexOf("xEditor-fontsize-") > -1 &&
    mark2Class.indexOf("xEditor-fontsize-") > -1
  ) {
...
const removeZeroWidthSpace = (root) => {
  walkTree(root, (node) => {
    ...
    if (
      ...
      [ANCHOR, FOCUS, CURSOR].indexOf(node.nextSibling.getAttribute(XEDITOR_ELEMENT)) >= 0
    ) {
      return;
    }
...
function k(node, root) {
  if (!Level[node.name] || !Level[root.name]) {
    return root;
  }
  if (Level[node.name] > Level[root.name]) {
    return node;
...
function w(nodeName, rootNodeName) {
  return (
    !(!["ol", "ul"].includes(nodeName) || "li" !== rootNodeName) ||
    !("blockquote" !== nodeName || !ROOT_TAG_MAP[rootNodeName] || "blockquote" === rootNodeName)
  );
}
  ...
  while (firstNode) {
    ...
    if (firstNode.isBlockCard() || firstNode.isTable() || firstNode.isSimpleBlock()) {
      ...
      while (firstNode) {
        ...
        if (isBR || !nextNode || nextNode.isBlock()) {
          break;
        }
        firstNode = nextNode;
      }
  ...
  while (firstChild) {
    ...
    while (
      ...
      ("blockquote" === firstChild.name ||
        (["ul", "ol"].includes(firstChild.name) && isSameList(firstChild, nextNode)))
    ) {

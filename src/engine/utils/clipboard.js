...
const writeClipboard = (event, parser, callback) => {
  ...
  if (parser !== null) {
    ...
    const cards = cardRoot.closest("[data-xEditor-card]", (node) => {
    ...
    const hasChildEngine =
      cardRoot.find(".xEditor-engine-view").length > 0 ||
      cardRoot.find(".xEditor-engine").length > 0;
    const hasPrentEngine =
      cardRoot.closest(".xEditor-engine-view").length > 0 ||
      cardRoot.closest(".xEditor-engine").length > 0;
...
const onCut = () => {
  ...
  const listElements =
    -1 !== ["ul", "ol"].indexOf(cardRoot.name) ? cardRoot : cardRoot.find("ul,ol");

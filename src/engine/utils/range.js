...
const shrinkRange = (range) => {
  ...
  while (
    ...
    range.endOffset > 0 &&
    (child = range.endContainer.childNodes[range.endOffset - 1]) &&
    child.nodeType === Node.ELEMENT_NODE &&
    !getNodeModel(child).isVoid() &&
    !getNodeModel(child).attr(CARD_KEY) &&
    !getNodeModel(child).attr(READY_CARD_KEY)
...
const createBookmark = (range) => {
  ...
  if (range.collapsed) {
    ...
    getNodeModel(cursor).attr(XEDITOR_ELEMENT, CURSOR);
  ...
  getNodeModel(anchor).attr(XEDITOR_ELEMENT, ANCHOR);
  ...
  getNodeModel(focus).attr(XEDITOR_ELEMENT, FOCUS);

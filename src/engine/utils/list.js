...
const isSameList = (sourceNode, targetNode) => {
  ...
  if (isTaskListBlock(sourceNode) !== isTaskListBlock(targetNode)) {
    return false;
  }
...
const isTaskListBlock = (listBlock) => {
  switch (listBlock.name) {
    case "li":
      return isCheckbox(listBlock.first()) || listBlock.hasClass("xEditor-list-task");

    case "ul":
      return listBlock.hasClass("xEditor-list");
...
const toTaskListBlock = function (listBlock, checked) {
  let indent;
  const taskUl = getNodeModel('<ul class="xEditor-list"/>');
  const taskLi = getNodeModel('<li class="xEditor-list-node xEditor-list-task"/>');

  switch (listBlock.name) {
    case "li":
      listBlock.addClass("xEditor-list-node");
      listBlock.addClass("xEditor-list-task");
      prependCheckbox.call(this, listBlock, checked);
      return listBlock;

    case "ul":
      listBlock.addClass("xEditor-list");
      return listBlock;
...
const clearTaskList = (node) => {
  switch (node.name) {
    case "li":
      node.removeClass("xEditor-list-node");
      node.removeClass("xEditor-list-task");
      removeCheckbox(node);
      return node;
    case "ul":
      node.removeClass("xEditor-list");
      return node;
    default:
      return node;
  }
...
const isCheckbox = (node) => {
  ...
  return (
    node.isCard() &&
    (node.attr("data-xEditor-card") === "checkbox" || "checkbox" === node.attr("data-ready-card"))
  );
};

function removeCheckbox(node) {
  node.find("[data-xEditor-card=checkbox]").remove();
...
function isListFirstOffset(range) {
  ...
  if (1 === contents.childNodes.length && "br" === getNodeModel(contents.firstChild).name) {
    return true;
  }
  if (1 === contents.childNodes.length && isCheckbox(getNodeModel(contents.firstChild))) {
    return true;
  }
  if (
    2 === contents.childNodes.length &&
    isCheckbox(getNodeModel(contents.firstChild)) &&
    isEmptyNode(contents.lastChild)
  ) {
    return true;
  }
  ...
  return (
    ...
    (!!isEmptyNode(node) ||
      (1 === node.children().length
        ? isCheckbox(node.first()) || "br" === node.first().name
        : 2 === node.children().length && isCheckbox(node.first()) && "br" === node.last().name))
  );
}

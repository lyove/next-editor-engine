...
export const transformCustomTags = (value) => {
  return value
    .replace(/<anchor\s*\/>/gi, "<span ".concat(XEDITOR_ELEMENT, '="').concat(ANCHOR, '"></span>'))
    .replace(/<focus\s*\/>/gi, "<span ".concat(XEDITOR_ELEMENT, '="').concat(FOCUS, '"></span>'))
    .replace(/<cursor\s*\/>/gi, "<span ".concat(XEDITOR_ELEMENT, '="').concat(CURSOR, '"></span>'))
    .replace(/(<card\s+[^>]+>).*?<\/card>/gi, (whole, tag) => {

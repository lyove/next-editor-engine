...
const template = () => {
  return '<div class="xEditor-embed-toolbar xEditor-embed-toolbar-active" contenteditable="false"></div>';
};
...
class EmbedToolbar {
  ...
  hide() {
    this.root.removeClass("xEditor-embed-toolbar-active");
  }

  show() {
    this.root.addClass("xEditor-embed-toolbar-active");
  }

  render(container) {
    const config = this.config;
    const group = getNodeModel('<div class="xEditor-embed-toolbar-group"></div>');
    ...
    if (config.type === "inline") {
      this.root.addClass("xEditor-embed-toolbar-inline");
    } else {
      this.root.addClass("xEditor-embed-toolbar-block");
    }

    if (["center", "right"].indexOf(config.align) >= 0) {
      this.root.addClass("xEditor-embed-toolbar-".concat(config.align));
    }

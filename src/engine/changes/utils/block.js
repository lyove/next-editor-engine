...
import { CURSOR_SELECTOR, XEDITOR_ELEMENT } from "../../constants/bookmark";
...
export const addOrRemoveBr = (range, align) => {
  ...
  block.find("br").each(function (br) {
    br = getNodeModel(br);
    if (
      ((!br.prev() || br.prev().attr(CARD_KEY) === "checkbox") &&
        br.next() &&
        br.next().name !== "br" &&
        br.next().attr(XEDITOR_ELEMENT) !== "cursor") ||
      (!br.next() && br.prev() && br.prev().name !== "br")
    ) {
  ...
  if (
    !block.first() ||
    (block.children().length === 1 && block.first().attr(CARD_KEY) === "checkbox")
  ) {
    block.append("<br />");
    return;
  }
  ...
  if (
    ...
    block.last().attr(XEDITOR_ELEMENT) === "cursor"
  ) {
...
export const brToParagraph = (block) => {
  ...
  if (
    block.children().length === 2 &&
    (block.first().attr(XEDITOR_ELEMENT) === "cursor" ||
      block.last().attr(XEDITOR_ELEMENT) === "cursor")
  ) {
    return;
  }

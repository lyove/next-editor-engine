...
function focusToCard(range, card) {
  ...
  if (cardRoot && card) card.focus(range, cardRoot, false);
}
...
function isListChild(_lastNode, _firstNode) {
  return (
    ...
    (_lastNode.name === _firstNode.name &&
      !("li" === _lastNode.name && !isSameList(_lastNode.parent(), _firstNode.parent())))
  );
}
...
function clearList(lastNode, nextNode) {
  if (lastNode.name === nextNode.name && "p" === lastNode.name) lastNode.attr(nextNode.attr());
  if (isTaskListBlock(lastNode) === isTaskListBlock(nextNode)) clearTaskList(nextNode);
}

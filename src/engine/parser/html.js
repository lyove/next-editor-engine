 ...
 * LowerValue: <p>foo</p><p><br /><span data-xEditor-element="cursor"></span></p>
 * DOM: HTML DOM tree
 * Markdown: ### heading
 * Text: plain text
 *
...
const walkTree = (node, conversionRules, callbacks, isCardNode) => {
  ...
  while (child) {
    if (child.isElement()) {
      ...
      delete attrs.style;
      // 光标相关节点
      if (attrs[XEDITOR_ELEMENT]) {
        name = attrs[XEDITOR_ELEMENT].toLowerCase();
        attrs = {};
        styles = {};
      }
      // 卡片相关节点
...
const pToDiv = (value) => {
  return value.replace(/<p(>|\s+[^>]*>)/gi, "<div$1").replace(/<\/p>/gi, "</div>");
};

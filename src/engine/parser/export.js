...
class Export {
  ...
  getDefaultCardParsers() {
    return {
      ...
      checkbox: (card) => {
        const checkbox = getNodeModel(
          "<span>".concat(
            "checked" === card.find("input").attr("checked") ? "\u2705" : "\ud83d\udd32",
            "<span/>"
          )
        );
        checkbox.css({
  ...
  transAttrsToStyle(card) {
    ...
    card.find(".xEditor-list-task").css({
      "list-style": "none",
    });
    ...
    card.find("blockquote").each((blockquote) => {
      ...
      if (blockquote.hasClass("xEditor-alert")) {
        blockquote.css({
          margin: 0,
          padding: "10px",
          "border-radius": "3px",
        ...
        if (blockquote.hasClass("xEditor-alert-info")) {
          blockquote.css({
            border: "1px solid #abd2da",
            "background-color": "#e8f7ff",
          });
        } else if (blockquote.hasClass("xEditor-alert-warning")) {
          blockquote.css({
            border: "1px solid #e0d1b1",
            "background-color": "#fffbe6",
          });
        } else if (blockquote.hasClass("xEditor-alert-danger")) {
          blockquote.css({
            border: "1px solid #deb8be",
            "background-color": "#fff3f3",
          });
        } else if (blockquote.hasClass("xEditor-alert-success")) {
          blockquote.css({
            border: "1px solid #c2d2b5",
            "background-color": "#edf9e8",
          });
        } else if (blockquote.hasClass("xEditor-alert-tips")) {
          blockquote.css({
  ...
  parseCard(root) {
    root.find("[data-xEditor-card]").each((card) => {
      card = getNodeModel(card);
      const type = card.attr("data-xEditor-card");
  ...
  parseLists(card) {
    card.find("ul,ol").each((list) => {
      ...
      const { name } = list;
      const indent = parseInt(list.attr("data-xEditor-indent"), 10) || 0;
      const styleTypes = {
        ul: ["disc", "circle", "square"],
        ol: ["decimal", "lower-alpha", "lower-roman"],
      };
      const styleType = styleTypes[name];
      list[0].removeAttribute("data-xEditor-indent");
      let tempList;
      for (let i = 0; i < indent; i++) {
        ...
        tempList.attr("xEditor-indent", i);
        tempList.css({
          margin: 0,
          "padding-left": "23px",
        });
  ...
  standardizeTaskList(card) {
    card.find(".xEditor-list-task").each((taskList) => {
      taskList = getNodeModel(taskList);
      const { childNodes } = taskList[0];
      const firstNode = childNodes[0];
      if (
        (childNodes.length === 0 || childNodes.length === 1) &&
        firstNode.nodeType === Node.ELEMENT_NODE &&
        "checkbox" === firstNode.getAttribute("data-xEditor-card")
      ) {
        taskList.remove();
      } else {
        const checkboxs = taskList.find('[data-xEditor-card="checkbox"]');
        ...
        if (checkboxs.length === 0) {
          const checked = "true" === taskList.attr("data-xEditor-checked") ? "true" : "false";
          taskList.prepend(
            '\n        <span data-card-type="inline" data-xEditor-card="checkbox" contenteditable="false" data-card-value="data:'
              .concat(
                checked,

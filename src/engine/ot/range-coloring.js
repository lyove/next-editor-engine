...
class RangeColoring {
  ...
  destroy() {
    ...
    parentNode.children(".xEditor-user-background").remove();
    parentNode.children(".xEditor-user-cursor").remove();
    parentNode.children(".xEditor-card-mask").remove();
  }
  ...
  drawBackground(range, options) {
    ...
    if (backgroundChild.length > 0) {
      ...
      backgroundChild = getNodeModel(
        '<div class="xEditor-user-background" data-uuid="'
          .concat(uuid, '" data-color="')
          .concat(color, '" />')
      );
      backgroundChild.css({
        position: "absolute",
  ...
  getNodeRect(node, rect) {
    ...
    if (node.isCard() && "checkbox" === node.attr("data-xEditor-card") && node.next()) {
      node = node.next();
    }
  ...
  setCursorRect(node, rect) {
    if (-1 !== rect.height) {
      if (0 === rect.height) {
        node.css(rect);
        node.addClass("xEditor-user-cursor-card");
        return;
      }
      node.css(rect);
      node.removeClass("xEditor-user-cursor-card");
    } else {
      node.remove();
    }
  }
  ...
  showCursorInfo(node, info) {
    ...
    trigger.addClass("xEditor-user-cursor-trigger-active");
  ...
  hideCursorInfo(node) {
    ...
    trigger.removeClass("xEditor-user-cursor-trigger-active");
  ...
  drawCursor(range, info) {
    ...
    let childCursor = parentNode.children('.xEditor-user-cursor[data-uuid="'.concat(uuid, '"]'));
    if (childCursor.length > 0) {
      this.setCursorRect(childCursor, cursorRect);
    } else {
      const userCursor = '\n      <div class="xEditor-user-cursor" data-uuid="'
        .concat(uuid, '">\n        <div class="xEditor-user-cursor-trigger">')
        .concat(escape(name), "</div>\n      </div>\n      ");
      childCursor = getNodeModel(userCursor);
      const trigger = childCursor.find(".xEditor-user-cursor-trigger");
      this.setCursorRect(childCursor, cursorRect);
  ...
  drawCardMask(node, cursor, info) {
    ...
    let mask = parentNode.children('.xEditor-card-mask[data-uuid="'.concat(info.uuid, '"]'));
    if (mask.length > 0) {
    ...
    mask = getNodeModel('<div class="xEditor-card-mask" data-uuid="'.concat(info.uuid, '" />'));
    mask[0].__targetNode = node[0];
    mask.css({
      left: nodeRect.left - parentRect.left + "px",
      top: nodeRect.top - parentRect.top + "px",
    ...
    mask.on("mousemove", (event) => {
      const embedElement = getNodeModel("div[data-xEditor-element=embed-tooltip]");
      embedElement.css({
  ...
  drawRange(range, info) {
    ...
    for (let i = 0; i < components.length; i++) {
      ...
      if (!cardInfo || node[0] !== cardInfo.cardRoot[0]) {
        ...
        parentNode.children('.xEditor-card-mask[data-uuid="'.concat(uuid, '"]')).remove();
      }
    }
    if (cardInfo) {
  ...
  updateBackgroundPosition() {
    const { parentNode } = this;
    parentNode.children(".xEditor-user-background").each((child) => {
  ...
  updateCursorPosition() {
    const { parentNode } = this;
    parentNode.children(".xEditor-user-cursor").each((child) => {
      const node = getNodeModel(child);
  ...
  updateCardMaskPosition() {
    const { parentNode } = this;
    const parentRect = parentNode[0].getBoundingClientRect();
    parentNode.children(".xEditor-card-mask").each((child) => {
      const node = getNodeModel(child);
  ...
  updateBackgroundAlpha(range) {
    const cursorRect = this.getCursorRect(range);
    this.parentNode.children(".xEditor-user-cursor").each((child) => {
      const node = getNodeModel(child);
      const trigger = node.find(".xEditor-user-cursor-trigger");
  ...
  render(data, members, n) {
    ...
    parentNode.children("[data-uuid]").each((child) => {
      ...
      const member = members.find((m) => m.uuid === uuid);
      if (!member || info[uuid]) {
        if (child.hasClass("xEditor-card-mask")) {

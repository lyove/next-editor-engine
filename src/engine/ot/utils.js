...
export const isTransientElement = (node) => {
  ...
  if (node.isElement()) {
    if (["cursor", "anchor", "focus"].includes(node.attr("data-xEditor-element"))) {
      return true;
    }
...
export const isTransientAttribute = (node, attr) => {
  node = getNodeModel(node);
  return (
    !(!node.isRoot() || /^data-selection-/.test(attr)) ||
    !(!node.isCard() || !["id", "class", "style"].includes(attr))
  );
};
...
export const toPath = (range) => {
  ...
  return [
    getPath(range.startContainer, range.startOffset),
    getPath(range.endContainer, range.endOffset),
  ];
};
